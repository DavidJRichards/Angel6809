...
;		ldd	#VRAM|0x0028	;initialize VRAM address
		stb	VDP_REG
		sta	VDP_REG
		jsr	cursor_invert
...
;;; - backspace
;;; - clear (Ctrl-L)
;;; arguments:	character in A	(NOTE, CHANGED FROM B REG)
;;; returns:	none
;;; destroys:	none
...
text_moveline	ldd	40,s		;get VRAM address
		...
		tfr	s,u
		lda	#40
t2d		ldb	,u+		;write the buffer back to VRAM
		stb	VDP_VRAM
		deca
		bne	t2d
...
str1		fcc	"MECB djrm TMS9918 Console / Assist09" 
		FCB	0

str4		fcc	"October 11th 2024"
		FCB	0
